...
test("SANKO publishes all seven current health programmes from reviewed curricula", async () => {
  const uid = "tr-sanko-universitesi";
  const programmes = Object.values(shards[uid]);
  const universityCoverage = coverage.universities.find((value) => value.universityId === uid);
  const sources = await data("turkey-catalog-sources-2026.json");

  assert.equal(programmes.length, 7);
  assert.equal(programmes.reduce((total, value) => total + value.courses.length, 0), 421);
  assert.deepEqual({
    structuredProgramCount: universityCoverage.structuredProgramCount,
    courseCount: universityCoverage.courseCount,
    missingProgramIds: universityCoverage.missingProgramIds,
  }, {
    structuredProgramCount: 7,
    courseCount: 421,
    missingProgramIds: [],
  });
  assert.deepEqual(sources[uid].catalogs.map((value) => value.url), [
    "https://www.sanko.edu.tr/fakulteler/saglik-bilimleri-fakultesi/saglik-bilimleri-fakultesi-ogretim-programi-mufredat/",
    "https://www.sanko.edu.tr/yuksekokul/saglik-hizmetleri-myo/saglik-hizmetleri-meslek-yuksek-okulu-ogretim-programimufredat/",
    "https://www.sanko.edu.tr/fakulteler/tip-fakultesi/ogretim-programi-mufredat/",
  ]);

  const ftr = shards[uid][`${uid}:program-osym-207710035`];
  assert.equal(ftr.courses.length, 92);
  assert.ok(ftr.courses.some((course) => course.code === "FTR403"
    && course.semester === 7 && course.kind === "required"));

  const nursing = shards[uid][`${uid}:program-osym-207710053`];
  assert.equal(nursing.courses.length, 90);
  assert.ok(nursing.courses.some((course) => course.code === "HEM422"
    && course.semester === 8 && course.kind === "required"));

  const operatingRoom = shards[uid][`${uid}:program-osym-207710109`];
  assert.equal(operatingRoom.courses.length, 36);
  assert.ok(operatingRoom.courses.some((course) => course.code === "MYO212"
    && course.semester === 4 && course.kind === "elective"));

  const anesthesia = shards[uid][`${uid}:program-osym-207710100`];
  assert.equal(anesthesia.courses.length, 45);
  assert.ok(anesthesia.courses.some((course) => course.code === "ANE203"
    && course.semester === 3 && course.kind === "required"));

  const imaging = shards[uid][`${uid}:program-osym-207710106`];
  assert.equal(imaging.courses.length, 44);
  assert.ok(imaging.courses.some((course) => course.code === "TGT207"
    && course.semester === 3 && course.kind === "required"));

  const emergency = shards[uid][`${uid}:program-osym-207710103`];
  assert.equal(emergency.courses.length, 41);
  assert.ok(emergency.courses.some((course) => course.code === "İAY203"
    && course.semester === 3 && course.kind === "required"));

  const medicine = shards[uid][`${uid}:program-osym-207710071`];
  assert.equal(medicine.courses.length, 73);
  assert.ok(medicine.courses.some((course) => course.code === "TIP556"
    && course.year === 5 && course.kind === "required"));
  assert.ok(medicine.courses.some((course) => course.code === "TIP631"
    && course.year === 6 && course.kind === "required"));
  assert.ok(medicine.courses.some((course) => course.code === "SEC601"
    && course.year === 6 && course.kind === "elective"));
});

test("Sivas Bilim ve Teknoloji publishes all eleven active UBYS curricula", async () => {
  const uid = "tr-sivas-bilim-ve-teknoloji-universitesi";
  const programmes = Object.values(shards[uid]);
  const universityCoverage = coverage.universities.find((value) => value.universityId === uid);
  const sources = await data("turkey-catalog-sources-2026.json");

  assert.equal(programmes.length, 11);
  assert.equal(programmes.reduce((total, value) => total + value.courses.length, 0), 1264);
  assert.deepEqual({
    structuredProgramCount: universityCoverage.structuredProgramCount,
    courseCount: universityCoverage.courseCount,
    missingProgramIds: universityCoverage.missingProgramIds,
  }, {
    structuredProgramCount: 11,
    courseCount: 1264,
    missingProgramIds: [],
  });
  assert.equal(sources[uid].catalogs[0].url,
    "https://ubys.sivas.edu.tr/AIS/OutcomeBasedLearning/Home/Index?culture=tr-TR");

  const computer = shards[uid][`${uid}:program-osym-111900101`];
  assert.equal(computer.courses.length, 125);
  assert.equal(computer.curriculumPeriod, "2024-2025 Bilgisayar Mühendisliği Öğretim Planı");
  assert.ok(computer.courses.some((course) => course.code === "CNG201"
    && course.semester === 3));
  assert.ok(computer.courses.some((course) => course.code === "CNG412"
    && course.semester === 8 && course.kind === "elective"));

  const drone = shards[uid][`${uid}:program-osym-111900129`];
  assert.equal(drone.courses.length, 59);
  assert.ok(drone.courses.some((course) => course.code === "İHA202"
    && course.semester === 4));

  const precisionAgriculture = shards[uid][`${uid}:program-osym-111900150`];
  assert.equal(precisionAgriculture.courses.length, 114);
  assert.equal(precisionAgriculture.curriculumPeriod,
    "2025-2026 Hassas Tarım ve Tarımsal Robotlar Öğretim Planı");
  assert.ok(precisionAgriculture.courses.some((course) => course.code === "HTTR203"
    && course.semester === 3));

  const space = shards[uid][`${uid}:program-osym-111900157`];
  assert.equal(space.courses.length, 126);
  assert.equal(space.curriculumPeriod, "2026 Öğretim Planı");
  assert.ok(space.courses.some((course) => course.code === "ANE408"
    && course.semester === 8 && course.kind === "elective"));

  const materials = shards[uid][`${uid}:program-osym-111900164`];
  assert.equal(materials.courses.length, 88);
  assert.ok(materials.courses.some((course) => course.code === "MME40X"
    && course.semester === 8));
  assert.ok(["MME312", "MME314", "MME316", "MME318", "MME320", "MME322", "MME323", "MME324"]
    .every((code) => materials.courses.every((course) => course.code !== code)));
});

test("Ahmet Yesevi publishes all seventeen campus and TÜRTEP study plans", async () => {
  const uid = "tr-ahmet-yesevi-universitesi";
  const programmes = Object.values(shards[uid]);
  const universityCoverage = coverage.universities.find((value) => value.universityId === uid);
  const sources = await data("turkey-catalog-sources-2026.json");

  assert.equal(programmes.length, 17);
  assert.equal(programmes.reduce((total, value) => total + value.courses.length, 0), 883);
  assert.deepEqual({
    structuredProgramCount: universityCoverage.structuredProgramCount,
    courseCount: universityCoverage.courseCount,
    missingProgramIds: universityCoverage.missingProgramIds,
  }, {
    structuredProgramCount: 17,
    courseCount: 883,
    missingProgramIds: [],
  });
  assert.ok(sources[uid].catalogs.some((value) => value.url.startsWith("https://ayu.edu.kz/")));
  assert.ok(sources[uid].catalogs.some((value) => value.url.startsWith("https://www.turtep.edu.tr/")));

  const literature = shards[uid][`${uid}:program-osym-403910273`];
  assert.equal(literature.courses.length, 55);
  assert.equal(literature.curriculumPeriod, "2025-2026");
  assert.ok(literature.courses.some((course) => course.code === "HOK1171"
    && course.semester === 1 && course.kind === "required"));
  assert.ok(literature.courses.some((course) => course.code === "TTMT4305"
    && course.semester === 8));

  const dentistry = shards[uid][`${uid}:program-osym-403990025`];
  assert.equal(dentistry.courses.length, 76);
  assert.ok(dentistry.courses.some((course) => course.code === "INT6501"
    && course.semester === 11 && course.kind === "required"));
  assert.ok(dentistry.courses.some((course) => course.code === "CP63120"
    && course.semester === 12));

  const ybs = shards[uid][`${uid}:program-osym-403910158`];
  assert.equal(ybs.courses.length, 49);
  assert.match(ybs.sourceUrl, /turtep\.edu\.tr/);
  assert.ok(ybs.courses.some((course) => course.code === "TİŞL-101"
    && course.semester === 1 && course.kind === "required"));
  assert.ok(ybs.courses.some((course) => course.code === "TYBS-408"
    && course.semester === 8 && course.kind === "required"));

  const programming = shards[uid][`${uid}:program-osym-403950015`];
  assert.equal(programming.courses.length, 28);
  assert.ok(programming.courses.some((course) => course.code === "TBPÖ-208"
    && course.semester === 4 && course.kind === "required"));
});

test("Mimar Sinan publishes eighteen exact programme curricula and keeps Psychology explicit", async () => {
  const uid = "tr-mimar-sinan-guzel-sanatlar-universitesi";
  const programmes = Object.values(shards[uid]);
  const universityCoverage = coverage.universities.find((value) => value.universityId === uid);
  const sources = await data("turkey-catalog-sources-2026.json");

  assert.equal(programmes.length, 18);
  assert.equal(programmes.reduce((total, value) => total + value.courses.length, 0), 1256);
  assert.deepEqual({
    structuredProgramCount: universityCoverage.structuredProgramCount,
    courseCount: universityCoverage.courseCount,
    missingProgramIds: universityCoverage.missingProgramIds,
  }, {
    structuredProgramCount: 18,
    courseCount: 1256,
    missingProgramIds: ["program-osym-107500101"],
  });
  assert.ok(sources[uid].catalogs.every((value) => value.url.startsWith("https://msgsu.edu.tr/")));

  const architecture = shards[uid][`${uid}:program-osym-107510092`];
  assert.equal(architecture.courses.length, 129);
  assert.ok(architecture.courses.some((course) => course.code === "MIM121"
    && course.semester === 1 && course.kind === "required"));

  const sociology = shards[uid][`${uid}:program-osym-107510056`];
  assert.equal(sociology.courses.length, 73);
  assert.ok(sociology.courses.some((course) => course.code === "SOS499"
    && course.semester === 8 && course.kind === "required"));

  const digitalGames = shards[uid][`${uid}:program-osym-107590323`];
  assert.equal(digitalGames.courses.length, 95);
  assert.equal(digitalGames.curriculumPeriod, "2026-2027");
  assert.ok(digitalGames.courses.some((course) => course.code === "DOT101"
    && course.semester === 1 && course.kind === "required"));
  assert.ok(digitalGames.courses.some((course) => course.code === "DOT358S"
    && course.semester === null && course.kind === "elective"));

  const philosophy = shards[uid][`${uid}:program-osym-107510117`];
  const artHistory = shards[uid][`${uid}:program-osym-107510047`];
  const clothing = shards[uid][`${uid}:program-osym-107550055`];
  const physics = shards[uid][`${uid}:program-osym-107510135`];
  assert.ok(philosophy.courses.every((course) => course.code !== "FEL-307"));
  assert.ok(artHistory.courses.every((course) => !["STA203", "STA333"].includes(course.code)));
  assert.ok(clothing.courses.every((course) => course.code !== "MYG111"));
  assert.ok(physics.courses.every((course) => course.code !== "FİZ522"));
});
