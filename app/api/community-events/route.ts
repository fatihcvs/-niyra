    ...
    const rows = await DB.prepare(
      `SELECT ce.id, ce.title, ce.description, ce.location, ce.starts_at, ce.ends_at, ce.capacity, ce.status,
       ...
       WHERE ce.community_id = ? AND ce.status IN ('active','cancelled')
         AND (${past ? "datetime(ce.starts_at) < datetime('now')" : "datetime(ce.starts_at) >= datetime('now', '-2 hours')"})
       ORDER BY CASE ce.status WHEN 'active' THEN 0 ELSE 1 END, ce.starts_at ${past ? "DESC" : "ASC"} LIMIT 30`,
    ).bind(identity.email, communityId).all<EventRow>();
    return Response.json({ events: rows.results.map(serialize) });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await DB.batch([
      DB.prepare(`INSERT INTO community_events (id, community_id, creator_email, title, description, location, starts_at, ends_at, capacity, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, communityId, identity.email, title, description, location, starts.toISOString(), ends?.toISOString() ?? null, capacity, now, now),
      DB.prepare(`INSERT INTO community_event_attendees (event_id, user_email, status) VALUES (?, ?, 'going')`).bind(id, identity.email),
      ...
      DB.prepare(
        `INSERT INTO notifications (id, user_email, actor_email, kind, title, body, entity_type, entity_id)
         ...
         WHERE cm.community_id = ? AND cm.status = 'active' AND cm.user_email <> ? AND COALESCE(np.communities, 1) = 1 AND cm.notification_level <> 'mute'`,
      ).bind(identity.email, `${manager.name} topluluğunda yeni etkinlik`, title, id, communityId, identity.email),
    ]);
    await audit(DB, identity.email, "community.event.created", "community-event", id, { communityId, startsAt: starts.toISOString() });
    await notify(DB, { userEmail: identity.email, actorEmail: identity.email, kind: "community", title: `${title} etkinliği oluşturuldu`, entityType: "community-event", entityId: id });
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (action === "cancel") {
      ...
      await DB.prepare(`UPDATE community_events SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP WHERE id = ?`).bind(id).run();
      await audit(DB, identity.email, "community.event.cancelled", "community-event", id, { communityId: event.community_id });
    ...
    await DB.prepare(`INSERT INTO community_event_attendees (event_id, user_email, status) VALUES (?, ?, ?) ON CONFLICT(event_id, user_email) DO UPDATE SET status = excluded.status, updated_at = CURRENT_TIMESTAMP`).bind(id, identity.email, going ? "not-going" : "going").run();
    const count = await DB.prepare(`SELECT COUNT(*) AS total FROM community_event_attendees WHERE event_id = ? AND status = 'going'`).bind(id).first<{ total: number }>();
    await audit(DB, identity.email, going ? "community.event.left" : "community.event.joined", "community-event", id, { communityId: event.community_id });
