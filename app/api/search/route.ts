...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [people, courses, posts, notes, communities] = await Promise.all([
      DB.prepare(
        `SELECT u.public_id, u.display_name, u.handle, d.name AS department_name, f.short_name AS faculty_short_name
         ...
         WHERE u.email <> ? AND u.status = 'active' AND sp.onboarding_completed = 1
           ...
           AND ${searchContainsSql("u.display_name || ' ' || u.handle || ' ' || d.name || ' ' || COALESCE(f.name, '')")}
      ...
      ).bind(identity.email, profile.university_id, scope, needle, identity.email, identity.email).all(),
      DB.prepare(
        `SELECT DISTINCT c.id, c.code, c.name, d.name AS department_name
         ...
         WHERE sp.university_id = ? AND ${searchContainsSql("c.code || ' ' || c.name || ' ' || d.name")}
         ORDER BY c.code LIMIT 6`,
      ).bind(profile.university_id, needle).all(),
      DB.prepare(
        `SELECT p.id, p.content, p.created_at, u.public_id AS author_id, u.display_name AS author_name, COALESCE(c.code, 'GENEL') AS course_code
         ...
         WHERE p.deleted_at IS NULL AND u.status = 'active'
           ...
           AND ${searchContainsSql("p.content || ' ' || COALESCE(c.code, '') || ' ' || u.display_name")}
      ...
      ).bind(scope, profile.university_id, scope, needle, identity.email, identity.email, identity.email, profile.university_id, identity.email, identity.email).all<{ id: string; content: string; created_at: string; author_id: string; author_name: string; course_code: string }>(),
      DB.prepare(
        `SELECT n.id, n.title, n.description, n.tags_json, n.created_at, c.code AS course_code,
         ...
         WHERE u.status = 'active' AND n.status = 'published' AND n.deleted_at IS NULL AND owner_profile.university_id = ?
           AND ${searchContainsSql("n.title || ' ' || n.description || ' ' || n.tags_json || ' ' || c.code || ' ' || c.name || ' ' || u.display_name")}
      ...
      ).bind(profile.university_id, needle, identity.email, identity.email).all<{ id: string; title: string; description: string; tags_json: string; created_at: string; course_code: string; owner_name: string; view_count: number }>(),
      DB.prepare(
        `SELECT c.id, c.name, c.description, c.category, c.slug,
         ...
         WHERE c.status = 'active' AND c.moderation_status = 'active' AND c.university_id = ?
           ...
           AND ${searchContainsSql("c.name || ' ' || c.description || ' ' || c.category")}
         ORDER BY member_count DESC, c.created_at DESC LIMIT 6`,
      ).bind(profile.university_id, identity.email, needle).all(),
    ]);
