...
export function SavedWorkspace({ renderPost, onNavigate }: { renderPost: (post: ProfilePost, onSaved: (saved: boolean) => void, onUpdated: (text: string) => void, onDeleted: () => void) => ReactNode; onNavigate: (name: string) => void }) {
  ...
  return <div className="workspace-view saved-workspace"><WorkspaceHeader section="Kaydedilenler" eyebrow="KİŞİSEL ARŞİVİN" title="Sonra dönmek istediklerin" description="Faydalı paylaşımlarını bul, yeniden oku veya kayıtlarından çıkar. Bu liste yalnızca sana görünür." actions={<RefreshButton busy={loading || loadingMore} onClick={() => { setLoading(true); setError(""); setRevision((value) => value + 1); }}/>} />
