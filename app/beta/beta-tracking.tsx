...
type Receipt = { request: Pick<BetaRequest, "id" | "kind" | "status" | "subject" | "message" | "playUrl" | "createdAt" | "updatedAt" | "expiresAt">; messages: BetaMessage[] };
...
export function BetaTracking() {
  ...
  return <>
    ...
    {data && <section className={styles.tracking} aria-live="polite"><span className={styles.badge}>{data.request.kind === "feedback" && data.request.status === "new" ? "Geri bildirim alındı" : BETA_STATUS[data.request.status]}</span><h2>{data.request.subject}</h2><p className={styles.messageText}>{data.request.message}</p>
      ...
      {data.request.status === "ready" && <p>Katılım koşullarını karşılayan başvurun test erişimi için sırada. Google Play erişimin açıldığında bağlantın burada yer alacak.</p>}
      {data.request.playUrl && ["invited", "testing", "completed"].includes(data.request.status) && <><a className={styles.primary} href={data.request.playUrl} target="_blank" rel="noopener noreferrer">Google Play testine katıl ↗</a><p>Başvuruda yazdığın Google hesabıyla aç ve “Test kullanıcısı ol” adımını tamamla. Listeye eklenmek tek başına teste katılmış olmak değildir.</p>{data.request.status === "invited" && <button className={styles.secondary} disabled={busy} onClick={() => void request("joined")}>Google Play’de teste katıldım</button>}</>}
      {data.request.status === "testing" && <p>Katıldığını bildirdin. Uygulamayı doğal biçimde kullanıp deneyimini paylaşabilirsin. Google Play test süresi Console üzerinden ayrıca doğrulanır.</p>}
