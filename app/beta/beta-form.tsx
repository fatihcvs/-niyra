...
function createToken() { return btoa(String.fromCharCode(...window.crypto.getRandomValues(new Uint8Array(32)))).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, ""); }

export function BetaForm({ kind }: { kind: "application" | "feedback" }) {
  const application = kind === "application";
  const [platform, setPlatform] = useState<"" | "web" | "android" | "both">("");
  const needsAndroid = application && (platform === "android" || platform === "both");
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (application && !["web", "android", "both"].includes(String(form.get("platform")))) {
      setError("Teste web, Android veya her ikisiyle katılacağını seç.");
      event.currentTarget.querySelector<HTMLSelectElement>('[name="platform"]')?.focus();
      return;
    }
    ...
    try {
      ...
      try { window.localStorage.setItem(BETA_RECEIPT_KEY, token.current); } catch { /* The visible receipt remains usable without storage. */ }
  ...
  if (receipt) return <div className={styles.success} role="status">
    ...
    <p>{application ? "Test hesabının etkinleştirme bilgileri e-posta ile iletilecek. Bağlantıyı açarak kendi parolanı oluşturabilirsin. Android seçtiysen Google Play katılımını ayrıca takip sayfandan izle." : "Ekibin yanıtını takip sayfanda görebilir ve görüşmeye devam edebilirsin."}</p>
  ...
  return <>
    <span className={styles.eyebrow}>{application ? "WEB VE ANDROID BETA · 18+" : "HATA, ÖNERİ VE DESTEK"}</span>
    <h1>{application ? "İlk deneyenlerden ol." : "Birlikte daha iyi yapalım."}</h1>
    <p className={styles.intro}>{application ? "Kampira’yı tarayıcıda, Android’de veya her ikisinde dene. E-posta adresini paylaş; test hesabını etkinleştirmek için kendi parolanı oluşturabileceğin bir bağlantı alacaksın." : "Bir sorun mu yaşadın, bir fikrin mi var? Giriş yapmadan bize yazabilir, takip kodunla yanıt alabilirsin."}</p>
    <form className={styles.form} onSubmit={event => void submit(event)}>
      {application && <label>Nereden katılmak istiyorsun?<select name="platform" required value={platform} onChange={event => setPlatform(event.target.value as typeof platform)} disabled={busy}><option value="" disabled>Bir seçenek seç</option><option value="web">Web — tarayıcıdan</option><option value="android">Android — Google Play kapalı testi</option><option value="both">Web ve Android</option></select><small className={styles.hint}>Yalnız web testi için Android cihaz veya 14 günlük katılım gerekmez.</small></label>}
      <div className={styles.twoColumns}><label>Adın (isteğe bağlı)<input name="displayName" autoComplete="given-name" maxLength={80}/></label><label>{application ? needsAndroid ? "Google Play hesabının e-postası" : "E-posta adresin" : "E-posta (isteğe bağlı)"}<input name="email" type="email" autoComplete="email" required={application} maxLength={254}/></label></div>
      {application && <label>Üniversiten (isteğe bağlı)<input name="university" maxLength={120} autoComplete="organization"/></label>}
      {(!application || needsAndroid) && <div className={styles.twoColumns}><label>Cihaz modeli{!application && " (isteğe bağlı)"}<input name="deviceModel" placeholder="Örn. Samsung Galaxy A54" required={needsAndroid} minLength={needsAndroid ? 2 : undefined} maxLength={120}/></label><label>Android sürümü (isteğe bağlı)<input name="androidVersion" placeholder="Örn. Android 14" maxLength={30}/></label></div>}
