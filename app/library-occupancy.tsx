...
export function LibraryOccupancyWorkspace({ universityShortName }: { universityShortName: string }) {
  ...
  return <div className="workspace-view library-live-workspace">
    ...
    {loading ? <div className="library-live-empty"><strong>Çalışma alanları hazırlanıyor…</strong></div> : areas.length === 0 ? <div className="library-live-empty"><span>İLK ALAN</span><strong>Kütüphane doluluk haritası henüz boş</strong><p>Bildiğin gerçek bir kat veya çalışma bölgesini ekle. Kapasiteyi bilmiyorsan boş bırak; Üniyra boş masa sayısı uydurmaz.</p><button type="button" onClick={() => setCreateOpen(true)}>İlk alanı ekle</button></div> : <div className="library-area-grid">{areas.map((area) => {
